...
def main():
    ...
    # Modified DataLoader setup for macOS compatibility
    num_workers = cfg['training']['n_workers']
    if num_workers > 0 and multiprocessing.get_start_method(allow_none=True) != 'fork':
        print("Forcing multiprocessing to use 'fork' start method")
        multiprocessing.set_start_method('fork', force=True)

    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=batch_size, 
        num_workers=num_workers if num_workers >=0 else 0,  # Handle negative values
        shuffle=True,
    ...
    val_loader = torch.utils.data.DataLoader(
            ...
            num_workers=cfg['training']['n_workers_val'] if cfg['training']['n_workers_val'] >=0 else 0,
            shuffle=False,
        collate_fn=data.collate_remove_none,
        worker_init_fn=data.worker_init_fn)
...
if __name__ == '__main__':
    # macOS-specific multiprocessing setup
